...
def train():
    ...
    for update in range(total_updates):
        ...
        for _ in range(n_steps):
            ...
            candidate_count = int(action_mask[:max_candidates].sum())
            if action_int == max_candidates:
                draw_count += 1
                if candidate_count > 0:
                    draw_with_candidates_count += 1
            ...
            if done:
                ...
                if info is not None and info["ppo_hand_count"] == 0:
                    win_count += 1
                current_episode_reward = 0.0
                current_episode_steps = 0
                env.reset()
        ...
        draw_with_candidates_rate = draw_with_candidates_count / float(n_steps)
        completed_episodes = len(episode_rewards)
        win_rate = win_count / completed_episodes if completed_episodes > 0 else float("nan")
        ...
        print(
            ...
            f"wins={win_count}/{completed_episodes}, "
            f"avg_episode_turns={avg_episode_turns:.2f}, "
            f"draw_count={draw_count}, "
            f"draw_rate={draw_rate:.2f}, "
            f"draw_with_candidates={draw_with_candidates_count}, "
            f"draw_with_candidates_rate={draw_with_candidates_rate:.2f}, "
