...
개선한 구현은 ILP를 유지하면서 먼저 최대 사용량 해를 찾은 뒤, 최대 사용량보다
정확히 1장과 2장 적게 사용하는 해를 강제해 전략적으로 다른 후보를 우선
생성한다. 이후 남은 슬롯은 같은 사용량의 대체 해로 보충한다.
...
최신 실험은 다목적 후보와 Reward V2를 사용하고, PPO가 두 좌석을 번갈아
학습하도록 설정했다. 후보 feature 차원이 변경된 모델은 기존 체크포인트를
이어 사용하지 않고 새로 학습한다.

```text
모델: rummikub_ppo_balanced_seat_model.pt
학습: 1,000 episodes, PPO 좌석 P0/P1 교대
평가: PPO 1,000 games + greedy baseline 1,000 games
```
...
PPO를 P0 좌석에서만 학습하던 문제를 수정하고, 매 에피소드마다 PPO 좌석을
P0과 P1 사이에서 교대했다. 500개의 동일한 패를 각 좌석에서 실행해 PPO와
greedy baseline을 평가했다. 따라서 각 정책은 1,000게임, 전체 평가는
2,000게임이다.

| 정책 | 승리 | 패배 | 타임아웃 | 승률 | 95% 신뢰구간 |
|---|---:|---:|---:|---:|---:|
| PPO + ILP candidates | 479 | 521 | 0 | 47.9% | 44.8% - 51.0% |
| Greedy top-1 + ILP | 493 | 507 | 0 | 49.3% | 46.2% - 52.4% |

PPO의 평균 최종 손패는 1.48장, 상대 ILP는 1.13장이었다. PPO의 승률은
greedy보다 1.4%p 낮았지만 신뢰구간이 겹쳤으므로 PPO가 유의미하게 약하거나
강하다고 단정할 수 없다. 이번 결과는 좌석 교대 이후에도 PPO의 추가적인
성능 향상을 확인하지 못한 것으로 해석한다.

평가 중 PPO 후보는 원본 해가 턴당 평균 4.77개였지만, 중복 제거 후 고유
후보는 평균 1.39개였다. 따라서 목적함수의 수에 비해 PPO가 실제로 선택할
수 있는 전략적 후보는 여전히 제한적이었다.

이후 후보 생성 코드를 다시 수정해 최대 사용량, 최대 사용량-1,
최대 사용량-2 타일을 정확히 사용하는 후보를 우선 생성하도록 했다. 이
변경은 코드와 테스트에 반영되었지만, 위 1,000판 결과에는 포함되지 않았으며
별도 재학습을 통해 효과를 검증해야 한다.
...
1,000에피소드 학습이 완료되면 PPO 1,000게임과 greedy baseline 1,000게임
평가를 자동으로 실행한다.
...
좌석 교대 학습을 적용한 1,000판 실험에서는 PPO 승률 47.9%, greedy 승률
49.3%를 기록했다. PPO가 greedy보다 유의미하게 강하다는 근거는 아직
확인하지 못했다.

현재 구현은 후보의 타일 사용량을 강제로 다르게 생성해 PPO의 실질적인
선택지를 늘리는 방향으로 수정되었다. 다음 실험에서는 이 후보 생성 변경이
실제 승률 향상으로 이어지는지 검증한다.
